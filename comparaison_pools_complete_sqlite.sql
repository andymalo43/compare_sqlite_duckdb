...
WHERE strftime('%Y', f.date_facture) = 2024 AND f.statut = 'PAYEE'
...
FROM ligne_facture lf
INNER JOIN facture f ON lf.facture_id = f.facture_id
WHERE strftime('%Y', f.date_facture) = 2025 AND f.statut = 'PAYEE'
GROUP BY lf.description
...
WHERE f.statut = 'PAYEE' 
  AND strftime('%Y', f.date_facture) = 2024
...
SELECT 
    ...
    c.date_creation
FROM client c
WHERE strftime('%Y', c.date_creation) <= 2024
...
SELECT 
    ...
    c.date_creation
FROM client c
WHERE strftime('%Y', c.date_creation) = 2025
...
WITH comparison_result AS (
    ...
    FROM client c
    INNER JOIN facture f ON c.client_id = f.client_id
    WHERE c.ville = 'Paris' AND f.statut = 'PAYEE' AND strftime('%Y', f.date_facture) = 2024
    ...
    FROM client c
    INNER JOIN facture f ON c.client_id = f.client_id
    WHERE c.ville = 'Lyon' AND f.statut = 'PAYEE' AND strftime('%Y', f.date_facture) = 2024
    GROUP BY c.client_id, c.nom
